...
First every source comes from a :ref:`publication` which could be a governmental agency, a media company (like The New York Times), an organization (such as Amnesty International), or an account on social media. These publications produce one or more :ref:`source`s which could be a web page, news article, book, report, video, or social media post. These sources are divided into one or more :ref:`citation` which show the specific part of the source that evidences a ``claim``. Claims are the basic building blocks of our data (see :ref:`Our General Data Model`).
