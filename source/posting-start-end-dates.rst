...
Security force personnel often have formalities tied to how they start or end a position, which must be captured in the data carefully to accurately establish when someone is or is not in their :ref:`posting`.
...
1) Appointment announcements are the most common source of information for establishing the start date of a person's :ref:`posting`. Usually, appointments are announced in government decrees or other official acts, or in reported in news media. These can also establish the end date for the person being replaced by the appointee (see below for examples).
...
Importantly, when there is sourcing for * both * an appointment date and a ceremony date we would have both entered into the dataset and simply enter ``conflict`` in the `status:meta <https://help.securityforcemonitor.org/en/latest/persons_postings.html#status-meta>`_ for the appointment date which by its very nature conflicts with the ceremony date.
...
.. admonition:: Example
    ...
    In this case the :ref:`posting` would have two different start dates, one using the date of the appointment of ``2014-08-01`` (with `status:meta <https://help.securityforcemonitor.org/en/latest/persons_postings.html#status-meta>`_ of ``conflict``) and the other for the date of the ceremony of ``2014-08-03`` (with `status:meta <https://help.securityforcemonitor.org/en/latest/persons_postings.html#status-meta>`_ of ``accepted``). 
...
.. admonition:: Example
    ...
    Both citation establish start dates for their respective postings (in other terms the ``starting:range`` for each :ref:`posting` is ``Y``). With no other citations or information in these citations data entry is simple.
...
.. admonition:: Example
    ...
    In this example the :ref:`posting` for ``John Smith`` still starts on ``2014-08-01`` (``starting:range`` for Smith's :ref:`posting` is ``Y``), and it also establishes the end for the :ref:`posting` of ``Christopher Butler`` on ``2014-08-01`` (in other terms the ``ending:range`` for Butler`s :ref:`posting` is ``Y``). Bulter would also have an imprecise range of ``since_beginning_of_time`` to ``2014-07-31`` (see :ref:`How Dates Work` for more guidance).
